...
import Foundation
import Combine // braucht man für observable
...
final class ListVM: ObservableObject{ // Observable = Änderungen werden sofort gemeldet, damit die Benutzeroberfläche automatisch aktualisiert wird
    ...
    func addItem(item: Item){
        switch item.status{
        case .todo: todoItems.append(item)
        case .inProgress: inProgressItems.append(item)
        case .done: doneItems.append(item)
        case .unknown: unknownItems.append(item)
        }
    }
    func deleteItem(item: Item){
        switch item.status{
        case .todo: do {
            for index in self.todoItems.indices{
                if self.todoItems[index] == item{
        ...
        case .inProgress: do{
            for index in self.inProgressItems.indices{
                if self.todoItems[index] == item{
                    self.inProgressItems.remove(at: index)
                    return
                }
            }
        }
        case .done: do{
            for index in self.doneItems.indices{
                if self.todoItems[index] == item{
        ...
        case .unknown: do{
            for index in self.unknownItems.indices{
                if self.todoItems[index] == item{
